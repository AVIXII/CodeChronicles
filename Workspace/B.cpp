...
void solve()
{
    ...
    long long n;
    cin >> n; // Value of n for this test case

    long long result = 0;
    long long power_of_two = 1; // Tracks the power of 2
    long long start = 1;        // Start of the current range

    for (int b = 1; start <= n; ++b)
    {
        // Determine the end of the current range
        long long end = min(n, (1LL << b) - 1);
        long long count = end - start + 1; // Number of numbers in this range

        // Update the result
        result = (result * mod_exp(power_of_two, count, MOD)) % MOD; // Shift the result left
        long long sum = ((start + end) * count / 2) % MOD;           // Sum of numbers in range
        result = (result + sum) % MOD;                               // Add current numbers
