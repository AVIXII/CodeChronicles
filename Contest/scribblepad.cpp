...
string yorn(bool check) { if (check) return "YES"; return "NO"; }
...
void extendedGCDHelper(ll a, ll b, vector<ll> &arr) { // Verified
    if (!b) {
        arr[0] = a;
        arr[1] = 1;
        arr[2] = 0;
...
ll mod_add(ll a, ll b, ll m) { a = a % m; b = b % m; return (((a + b) % m) + m) % m; } // Verified

ll mod_sub(ll a, ll b, ll m) { a = a % m; b = b % m; return (((a - b) % m) + m) % m; } // Verified

ll mod_mul(ll a, ll b, ll m) { a = a % m; b = b % m; return (((a * b) % m) + m) % m; } // Verified
...
ll minv(ll a, ll b, bool b_is_prime = false) { // O(log(n))
    ...
    assert(ans[0] == 1 && "GCD is not equal to 1. Modulo inverse does not exist");
    return (ans[1] % b + b) % b;
}
...
void precal() { // Before the start of t testcases
}
